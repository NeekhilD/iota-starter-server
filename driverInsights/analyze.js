...
_.extend(driverInsightsAnalyze, {
	...
	_sendJobRequest: function (from, to) {
		...
		var options = {
			method: 'POST',
			url: config.baseURL + api + (config.tenant_id ? '?tenant_id=' + config.tenant_id : ""),
			headers: {
	...
	getSummary: function (trip_id) {
		...
		let params = [];
		if (config.tenant_id) {
			params.push('tenant_id=' + config.tenant_id);
		}
		if (trip_id) {
	...
	_getDetail: function (trip_id, tripuuid) {
		if (tripuuid) {
			...
			var options = {
				method: 'GET',
				url: config.baseURL + api + '?trip_uuid=' + tripuuid + (config.tenant_id ? '&tenant_id=' + config.tenant_id : "")
			};
			_.extend(options, this.authOptions);
			request(options, function (error, response, body) {
				if (!error && response.statusCode === 200) {
	...
	*/
	_run: function (method, api, uriParam, body, callback, errorback) {
		...
		uriParam = uriParam || [];
		if (config.tenant_id) {
			uriParam["tenant_id"] = config.tenant_id;
		}
		if (Object.keys(uriParam).length > 0) {
